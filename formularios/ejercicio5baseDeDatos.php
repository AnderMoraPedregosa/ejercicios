<?php

$productos = 
[
    "001" => 
    [
        "nombre" => "Logitech K120",
        "descripción" => "Teclado multimedia USB plug&play, trackPoint Caps (10pk, Soft Dome)",
        "precio" => 25.99
    ],
    "002" =>
     [
        "nombre" => "Lenovo LI5",
        "descripción" => "El mouse Lenovo 300 compacto inalambrico es el accesorio perfecto para cualquier persona que desee un mayor control y libertad",
        "precio" => 12.99
    ],
    "003" => 
    [
        "nombre" => "Monitor LG X10",
        "descripción" => "LCD con retroiluminación LED ThinkVision T1714p Square de 17 pulgadas",
        "precio" => 179.99
    ],
    "004" =>
     [
        "nombre" => "Monitor Lenovo Q24i",
        "descripción" => "Pantalla de 60,45 cm (23,8) Funciones como AMD FreeSync",
        "precio" => 172
    ],
    "005" => 
    [
        "nombre" => "ThinkPad X1 Extreme",
        "descripción" => "ThinkPad X1 Extreme de 2.ª generacion gestiona exigentes tareas informaticas sin problemas. Con pantalla tactil 4K OLED",
        "precio" => 1200
    ]
];